...
归一化波束成形增益图（理论计算，非训练结果）。

论文原文事实：

- 纵轴：Beamforming Gain（远场波束成形向量下的归一化增益 `|b^H a|`）
- 横轴：Distance (m)，约 0~200 m
- 门限：`Δ = 0.1`（即增益损失 10%，对应 `1-Δ = 0.9`）
- 趋势：随距离增加先下降后上升
- 含义：水平面 ENFR 位于两个远场区域之间，验证 Lemma 1
- 系统参数（仿真设置）：`N=256, h_B=15 m, θ_tit=5°, fc=30 GHz, d=λ/2=0.5 cm`
- 地面用户坐标 `(x_k, 0)`；`θ_k = arctan(|h_k-h_B|/x_k)`，相对角 `θ = θ_k - θ_tit`
- 增益计算：`μ(θ,r) = |b(θ,r)^H a(θ)|`，与公式 (3)(5)(6)(8) 一致
...
- 对象：所提出模型（Proposed / GPT-2 方案），非 CNN
- 训练轮数：500
- 约 30 epoch 后训练损失逐渐收敛
- 模型选择：按最低验证损失选择最终模型
- 论文示例：Epoch 304 取得最低验证损失
- 训练损失总体随 epoch 增加而下降
