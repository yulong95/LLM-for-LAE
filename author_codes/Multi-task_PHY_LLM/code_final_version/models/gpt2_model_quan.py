...
class Gpt2Model(nn.Module):
    ...
    def init_llm(self,gpt2_model_path):
        ...
        gpt2 =  GPT2Model.from_pretrained(gpt2_model_path,torch_dtype=torch.float,quantization_config=BitsAndBytesConfig(
                load_in_4bit=True,
        ...
        gpt2 = get_peft_model(gpt2, loftq_config)
        loftq_weight_path = os.path.join(gpt2_model_path, 'loftq_init', 'adapter_model.bin')
        adapters_weights = torch.load(loftq_weight_path, map_location='cpu')
        gpt2.load_state_dict(adapters_weights, strict=False)
        logging.info('Loaded LoftQ initialized weights')
        gpt2.train()
