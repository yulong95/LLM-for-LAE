function [mu,cor,index_A,h_o,kmax,optimal_index,GPR_train_number] = GPR_beamtraining_2_2(Kernal,sigma2,Observe_step,S,max_GPR_iter,QUA,Gain_vector,i_max,alpha)
index_A = [];                % Index of observed points
A_t = [];                    % Observed points
h_o = [];                    % Value of observed points

alpha_1 = sqrt(alpha/(alpha + 1));
alpha_2 = sqrt(1/(alpha + 1));
...
for aa = 1:max_GPR_iter
    if aa == 1
        ...
        for bb =1:Candidate_num
            ...
            Candidate_value(bb) = alpha_1*mu_temp + alpha_2*abs(Cov_temp);
        end
