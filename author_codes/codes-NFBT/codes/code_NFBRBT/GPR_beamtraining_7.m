function [mu,cor,index_A,h_o,kmax] = GPR_beamtraining_6(Kernal,sigma2,Observe_step,S,max_GPR_iter,QUA,Gain_vector)
...
for aa = 1:max_GPR_iter
    ...
    k_t = Kernal(index_A,:);
    mu = k_t'*(K_t+0.001*sigma2*eye(Observed_length))^(-1)*(h_o);
    Sigma_k = Kernal - k_t'*(K_t+0.001*sigma2*eye(Observed_length))^(-1)*k_t;
