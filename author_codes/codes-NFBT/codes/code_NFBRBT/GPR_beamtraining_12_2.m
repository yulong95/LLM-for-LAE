function [mu,cor,index_A,h_o,kmax] = GPR_beamtraining_12_2(Kernal,sigma2,Observe_step,S,max_GPR_iter,Gain_vector)
...
for aa = 1:max_GPR_iter
    ...
    mu = k_t'*((K_t+lambda*eye(Observed_length))\h_o);
    Sigma_k = Kernal - k_t'*(K_t+lambda*eye(Observed_length))^(-1)*k_t;
