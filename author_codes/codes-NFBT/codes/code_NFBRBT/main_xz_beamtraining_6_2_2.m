...
alpha_index = linspace(0.01,1,100);
...
rate_far = zeros(alpha_length,sample,length(SNR_dB));
rate_near = zeros(alpha_length,sample,length(SNR_dB));
rate_opt = zeros(alpha_length,sample,length(SNR_dB));
rate_far_and_near = zeros(alpha_length,sample,length(SNR_dB));
rate_near_hierarchical = zeros(alpha_length,sample,length(SNR_dB));
rate_near_GPR_3 = zeros(alpha_length,sample,length(SNR_dB));
...
%% generate the Kernal
   ...
   GPR_train_num_average = zeros(alpha_length,sample,length(SNR_dB));
   H_index = zeros(alpha_length,N,sample);
   theta_H_index = zeros(alpha_length,sample,1);
   r_H_index = zeros(alpha_length,sample,1);


%% training

for aaa = 1:alpha_length
    aaa
    alpha = alpha_index(aaa);

for t = 1:sample
    t   
    [H, hc, r, theta, G] = near_field_channel(N, K, L, d, fc, fs, M, Rmin, Rmax,sector, 1);
    theta_H_index(aaa,t,1) = theta;
    r_H_index(aaa,t,1) = r;
    H = channel_norm(H);
    k = 1;
    Hsf =  reshape(H(k, :, :), [N, M]);    
   ...
   for s = 1:length(SNR_dB)
    ...
    rate_far(aaa,t,s) = log2(1 + SNR * array_gain_far);
    ...
    rate_near(aaa,t,s) = log2(1 + SNR * array_gain_near);
    %% perfect CSI based beamforming
    wc_opt = exp(1j*angle(Z'))/sqrt(N);
    array_gain = abs(wc_opt*Z)^2;
    rate_opt(aaa,t,s) = log2(1 + SNR * array_gain);
 
    %% GPR_based_near_field beam training
    max_GPR_iter = 1000;
    [mu_3,cor_3,index_A_3,h_o_3,kmax_3,GPR_index_3,GPR_train_number] = GPR_beamtraining_2_2(Kernal_SV_mean,SNR,1,S,max_GPR_iter,QUA,Gain_vector,i_max,alpha);
    GPR_train_num_average(aaa,t,s) = GPR_train_number;
    array_gain_near_GPR_3= abs(QUA(GPR_index_3,:)*Z)^2;
    rate_near_GPR_3(aaa,t,s) = log2(1 + SNR * array_gain_near_GPR_3);
   end
end
