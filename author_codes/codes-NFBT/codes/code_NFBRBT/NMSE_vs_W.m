...
W_num = [10:10:100];       % Aperture length per wavelength
% W_num(1) = 10;
...
for qq = 1:Length
    ...
    load(['Kernel/Kernal' num2str(W_num(qq)) '.mat']);
...
save('NMSE_vs_W.mat','W_num','NMSE_OMP','NMSE_ML','NMSE_LMMSE','NMSE_GPR_exp','NMSE_GPR');
...
plot(W_num,NMSE_OMP,'-s','LineWidth',1.5,'Color',C(4,:));
plot(W_num,NMSE_ML,'-o','LineWidth',1.5,'Color',C(3,:));
plot(W_num,NMSE_LMMSE,'-p','LineWidth',1.5,'Color',C(5,:));
plot(W_num,NMSE_GPR_exp,'-r>','LineWidth',1.5,'Color',C(2,:));
plot(W_num,NMSE_GPR,'-d','LineWidth',1.5,'Color',C(1,:));
%legend('MAS-OMP','MAS-ML','SBR (${\bf \Sigma} = {\bf \Sigma}_{\rm exp}$)','SBR (${\bf \Sigma} = {\bf \Sigma}_{\rm bes}$)','SBR (${\bf \Sigma} = {\bf \Sigma}_{\rm cov}$)','FontSize',12,'Interpreter','latex');
legend('MAS-OMP','MAS-ML','SeLMMSE','SBR (${\bf \Sigma} = {\bf \Sigma}_{\rm exp}$)','SBR (${\bf \Sigma} = {\bf \Sigma}_{\rm cov}$)','FontSize',12,'Interpreter','latex');
xlabel('Length of flexible antenna array $(\lambda)$','Interpreter','latex','FontSize',14);
