...
parfor rp = 1:Rep
    ...
    for qq = 1:Length
        [h_hat_GPR,~,~,~] = GPR_multi(x,h,Kernal_SV_mean, sigma2, P(qq)*N_ports, 1);
...
save('NMSE_vs_P_SV.mat','P','NMSE_OMP','NMSE_ML','NMSE_LMMSE','NMSE_GPR_exp','NMSE_GPR','NMSE_GPR_J0');
...
plot(P,NMSE_GPR_exp,':o','LineWidth',1.5,'Color',C(2,:));
plot(P,NMSE_GPR_J0,'-g>','LineWidth',1.5,'Color',C(2,:));
...
legend('FAS-OMP','FAS-ML','SeLMMSE','Proposed $\bar S$ (${\bf \Sigma}_{\rm exp}$)','Proposed $\bar S$ (${\bf \Sigma}_{\rm bes}$)','Proposed $\bar S$ (${\bf \Sigma}_{\rm cov}$)','FontSize',12,'Interpreter','latex');
