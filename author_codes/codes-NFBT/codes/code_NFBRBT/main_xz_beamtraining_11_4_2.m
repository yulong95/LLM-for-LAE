%% far_field GPR_beamtraining success
clc;
clear all
close all
...
for t = 1:sample
   ...
   for s = 1:length(SNR_dB)
    ...
    elseif (GPR_index >= 1) && (GPR_index <= 4)
    ...
    elseif (GPR_index <= S) && (GPR_index >= (S-4))
    ...
    % fine search 2 end
    array_gain_far_GPR = max(abs(conj(Codebook_far(GPR_index,:))*H)^2,Gain_baseline);
