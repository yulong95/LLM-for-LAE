%% far_field GPR_beamtraining success 20240816
clc;
clear all
close all
...
%% generate the Kernal
   ...
   for aa = 1:S
    for bb = 1:S
        ...
        Kernal_exp(aa,bb) = exp(-norm(col(bb)-col(aa))^2/(2/D));
    end
   end
...
for t = 1:sample
   ...
   for s = 1:length(SNR_dB)
    ...
    %% GPR_based_far_field beam training_ei
    max_GPR_iter = 60;
    % [mu_3,cor_3,index_A_3,h_o_3,kmax_3] = GPR_beamtraining_11_2(Kernal_exp,SNR,1,S,max_GPR_iter,Codebook_far,Gain_vector);
    % [mu_3,cor_3,index_A_3,h_o_3,kmax_3] = GPR_beamtraining_11_4(Kernal_exp,SNR,1,S,max_GPR_iter,Codebook_far,Gain_vector);
    [mu_3,cor_3,index_A_3,h_o_3,kmax_3] = GPR_beamtraining_farfield_ei(Kernal_exp,SNR,1,S,max_GPR_iter,Codebook_far,Gain_vector);
    % [mu_3,cor_3,index_A_3,h_o_3,kmax_3] = GPR_beamtraining_farfield_gpucb(Kernal_exp,SNR,1,S,max_GPR_iter,Codebook_far,Gain_vector);
    mu_baseline = 0;
    for i = 1:S
    ...
    for i = (GPR_index-4):(GPR_index+5)
        if Gain_baseline <= abs(conj(Codebook_far(i,:))*H)^2
           Gain_baseline = abs(conj(Codebook_far(i,:))*H)^2;
           GPR_index = i;
       end
    end
    ...
    % fine search 2 end
    array_gain_far_GPR_ei = max(abs(conj(Codebook_far(GPR_index,:))*H)^2,Gain_baseline);
    % array_gain_far_GPR = abs(conj(Codebook_far(GPR_index,:))*H)^2;
    rate_GPR_ei(t,s) = log2(1 + SNR * array_gain_far_GPR_ei);
    
    %% GPR_based_far_field beam training_gpucb
    max_GPR_iter = 60;
    % [mu_3,cor_3,index_A_3,h_o_3,kmax_3] = GPR_beamtraining_11_2(Kernal_exp,SNR,1,S,max_GPR_iter,Codebook_far,Gain_vector);
    % [mu_3,cor_3,index_A_3,h_o_3,kmax_3] = GPR_beamtraining_11_4(Kernal_exp,SNR,1,S,max_GPR_iter,Codebook_far,Gain_vector);
    % [mu_3,cor_3,index_A_3,h_o_3,kmax_3] = GPR_beamtraining_farfield_ei(Kernal_exp,SNR,1,S,max_GPR_iter,Codebook_far,Gain_vector);
    ...
    % fine search 2 end
    array_gain_far_GPR_gpucb = max(abs(conj(Codebook_far(GPR_index,:))*H)^2,Gain_baseline);
    % array_gain_far_GPR = abs(conj(Codebook_far(GPR_index,:))*H)^2;
    rate_GPR_gpucb(t,s) = log2(1 + SNR * array_gain_far_GPR_gpucb);
...
plot(SNR_dB,mean(rate_GPR_ei),'gp-','Linewidth', 1.6)
plot(SNR_dB,mean(rate_GPR_gpucb),'cp-','Linewidth', 1.6)
plot(SNR_dB,mean(rate_perfect),'bd-', 'Linewidth', 1.6)
legend('Far-field exhaustive beam training ', 'GPR based far field beam training(ei) ','GPR based far field beam training(GP-UCB) ','Perfect CSI beamforming')
xlabel('SNR (dB)');
